...
void create_warpatch_process(char *dll_name)
{
	STARTUPINFOA		si;
	...
	Addr = (PVOID)VirtualAllocEx(pi.hProcess, 0, strlen(dll_name) + 1, MEM_COMMIT, PAGE_READWRITE);
	if (Addr == NULL)
	{
	...
	WriteProcessMemory(pi.hProcess, Addr, (void*)dll_name, strlen(dll_name) + 1, NULL);
...
int main(int argc, char **argv)
{
	...
	create_warpatch_process(argv[1]);
	return 0;
}